#!/usr/bin/env python
"""Train a Siamese CNN student on Borzoi SNP delta labels."""
...
from pocketreg.models.delta_siamese_cnn import SiameseDeltaCNN  # noqa: E402
...
LOGGER = logging.getLogger("train_borzoi_delta_student")
...
def build_metadata(row: dict[str, Any]) -> np.ndarray:
    dist = float(row.get("distance_to_tss", 0.0))
    gene_len = max(1.0, float(row.get("gene_end", 0.0)) - float(row.get("gene_start", 0.0)))
    rel = 0.0
    if row.get("gene_start") is not None and row.get("gene_end") is not None:
        rel = (float(row["pos_0based"]) - float(row["gene_start"])) / gene_len
    strand = 1.0 if row.get("strand") == "+" else -1.0
    bases = ["A", "C", "G", "T"]
    ref = [1.0 if row.get("ref") == b else 0.0 for b in bases]
    alt = [1.0 if row.get("alt") == b else 0.0 for b in bases]
    return np.array(
        [
            dist / 100000.0,
...
class DeltaDataset:
    ...
    def __getitem__(self, index: int):
        ...
        if observed != str(row["ref"]).upper():
            raise ValueError(
                f"REF mismatch for {row.get('variant_example_id')}: expected {row['ref']}, observed {observed}"
            )
        alt_seq = ref_seq[:center] + str(row["alt"]).upper() + ref_seq[center + 1 :]
        y = float(row["delta_teacher"])
        y_std = (y - self.target_mean) / self.target_std if self.standardize else y
        metadata = build_metadata(row) if self.metadata_features else np.zeros((0,), dtype=np.float32)
        return (
            torch.from_numpy(one_hot_encode(ref_seq, channels_first=True)),
            torch.from_numpy(one_hot_encode(alt_seq, channels_first=True)),
            torch.from_numpy(metadata),
            torch.tensor(y_std, dtype=torch.float32),
            row["variant_example_id"],
            y,
            row.get("example_id", ""),
        )
...
def compute_metrics(y_true: list[float], y_pred: list[float]) -> dict[str, float]:
    ...
    sign_acc = float(np.mean(np.sign(y_true_arr) == np.sign(y_pred_arr))) if y_true else float("nan")
    return {
...
def top_effect_metrics(frame: pd.DataFrame, quantiles: list[float]) -> dict[str, dict[str, float]]:
    ...
    for q in quantiles:
        ...
        out[f"top_{int(q * 100)}"] = compute_metrics(sub["y_true"].tolist(), sub["y_pred"].tolist())
        out[f"top_{int(q * 100)}"]["n"] = int(len(sub))
        out[f"top_{int(q * 100)}"]["cutoff_abs_delta"] = cutoff
    return out
...
def evaluate(model, loader, device, target_mean: float, target_std: float, standardize: bool):
    ...
    with torch.no_grad():
        for ref, alt, meta, _, ids, y_raw, ex_ids in loader:
            ref = ref.to(device=device, dtype=torch.float32, non_blocking=True)
            alt = alt.to(device=device, dtype=torch.float32, non_blocking=True)
            meta = meta.to(device=device, dtype=torch.float32, non_blocking=True)
            pred = model(ref, alt, meta if meta.shape[1] else None).detach().cpu().numpy()
            if standardize:
                pred = pred * target_std + target_mean
            y_pred.extend(float(v) for v in pred)
            y_true.extend(float(v) for v in y_raw)
            variant_ids.extend(str(v) for v in ids)
            example_ids.extend(str(v) for v in ex_ids)
...
def save_plots(plot_dir: Path, history: list[dict[str, Any]], predictions: dict[str, pd.DataFrame]) -> None:
    ...
    for split, frame in predictions.items():
        ...
        plt.scatter(frame["y_true"], frame["y_pred"], s=4, alpha=0.25)
...
def main() -> None:
    ...
    assert_compute_context("train_borzoi_delta_student", allow_local=allow_local, toy=toy)
    ...
    run_name = args.run_name or config.get("run_name", "k562_delta_student")
    ...
    context_len = int(data_cfg.get("local_context_len", 8192))
    standardize = bool(data_cfg.get("target_standardize", True))
    metadata_features = bool(data_cfg.get("metadata_features", True))
    train_y = np.array([float(row["delta_teacher"]) for row in rows_by_split["train"]], dtype=float)
    target_mean = float(np.mean(train_y))
    target_std = float(np.std(train_y)) or 1.0
    metadata_dim = len(build_metadata(rows_by_split["train"][0])) if metadata_features else 0
    ...
    for epoch in range(1, max_epochs + 1):
        model.train()
        losses: list[float] = []
        for ref, alt, meta, y, _, _, _ in loaders["train"]:
            ref = ref.to(device=device, dtype=torch.float32, non_blocking=True)
            alt = alt.to(device=device, dtype=torch.float32, non_blocking=True)
            meta = meta.to(device=device, dtype=torch.float32, non_blocking=True)
            y = y.to(device=device, dtype=torch.float32, non_blocking=True)
            optimizer.zero_grad(set_to_none=True)
            pred = model(ref, alt, meta if meta.shape[1] else None)
            loss = loss_fn(pred, y)
            loss.backward()
            if grad_clip > 0:
        ...
        scheduler.step()
        val_metrics, _ = evaluate(model, loaders["val"], device, target_mean, target_std, standardize)
        record = {
            "epoch": epoch,
        ...
        LOGGER.info(
            "epoch=%s train_loss=%.6f val_pearson=%.4f val_spearman=%.4f",
            epoch,
            record["train_loss"],
            val_metrics["pearson"],
    ...
    for split, loader in loaders.items():
        split_metrics, frame = evaluate(model, loader, device, target_mean, target_std, standardize)
        metrics[split] = split_metrics
        metrics[f"{split}_top_effect"] = top_effect_metrics(frame, quantiles)
